...
void Player::displayNext5Tiles(const std::vector<Tile>& allTiles) const {
    std::cout << "Next tiles:\n";

    for (int i = 0; i < std::min(5, static_cast<int>(allTiles.size())); ++i) {
        std::cout << "Next tiles (" << i + 1 << "):\n";
        allTiles[allTiles.size() - 1 - i].display();
    }
}


void Player::displayBoard(const Board& gameBoard) const {
